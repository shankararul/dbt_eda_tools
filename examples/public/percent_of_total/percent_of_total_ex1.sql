SELECT
    ...
    -- example: sum
    , {{percent_of_total('str_length','sum',3)}} AS sum_percent
    ...
    , {{percent_of_total('company_name', precision=3)}} AS count_percent

    , COUNT(DISTINCT company_name) AS count_distinct_company_name
    -- example: countdistinct
    , {{percent_of_total('company_name','countdistinct', precision=3)}} AS count_distinct_percent
